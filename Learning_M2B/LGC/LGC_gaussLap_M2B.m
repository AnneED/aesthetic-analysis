...
% LGC 
% The similarity matrix is based on gaussian feature similarity + score similarity
...
save('results_LGC_gauss_eM2B.mat', 'MAE', 'PC', 'RMSE');
...
save('results_LGC_gauss_wM2B.mat', 'MAE', 'PC', 'RMSE');
...
save('results_LGC_gauss_bM2B.mat', 'MAE', 'PC', 'RMSE');
...
load('results_LGC_gauss_eM2B.mat');
...
load('results_LGC_gauss_wM2B.mat');
[mae, idx] = min(mean(MAE));
mae
rmse = mean(RMSE(:, idx))
pc = mean(PC(:, idx))
...
load('results_LGC_gauss_bM2B.mat');
[mae, idx] = min(mean(MAE));
mae
rmse = mean(RMSE(:, idx))
pc = mean(PC(:, idx))
