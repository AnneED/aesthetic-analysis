% FME 						
% The Laplacian is based on cosine feature similarity + score similarity
   ...
   for j = 1:7
       for k = 1:7
           ...
           parfor l = 1:10
               ...
               T = labels;    
               T(unlabeled) = 0;
               W = Cos_GraphConstruction4(X_n, epsilon, T, devs);
               L = double(diag(sum(W)) - W) ;   
               [WW, b, F] = FME_semi2(X_n, L, T, para);
...
save('results_FME_50cos_vgg7.mat', 'MAE', 'PC', 'RMSE', 'E');
...
save('results_FME_70cos_vgg7.mat', 'MAE', 'PC', 'RMSE', 'E');
...
save('results_FME_90cos_vgg7.mat', 'MAE', 'PC', 'RMSE', 'E');



%% Results
load('results_FME_50cos_vgg7.mat');
...
load('results_FME_70cos_vgg7.mat');
[mae, idx] = min(mean(MAE));
mae
rmse = mean(RMSE(:, idx))
pc = mean(PC(:, idx))
...
load('results_FME_90cos_vgg7.mat');
[mae, idx] = min(mean(MAE));
mae
rmse = mean(RMSE(:, idx))
pc = mean(PC(:, idx))
